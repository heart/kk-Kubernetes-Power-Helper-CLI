...
| `delete`      | `kk delete <pod-pattern>`                                                             | Delete a pod whose name matches `pod-pattern`. Uses the same pattern-based pod selection, resolves to a single pod via `fzf` or an index picker if needed, then runs `kubectl delete pod`.                                                                                                                               |
